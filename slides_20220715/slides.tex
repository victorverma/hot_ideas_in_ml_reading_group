...
\begin{frame}{Gaussian Priors}
    Let $f(\boldsymbol{x}; \boldsymbol{\theta})$ be a $L$-layer MLP with weight matrices $\boldsymbol{W_1}, \ldots, \boldsymbol{W_L}$ and bias vectors $\boldsymbol{b_1}, \ldots, \boldsymbol{b_L}$. Specifying a prior entails specifying the distributions of the $\boldsymbol{W}_{\ell}$'s and the $\boldsymbol{b}_{\ell}$'s. Typically, Gaussian priors are used: $\boldsymbol{W}_{\ell} \sim \mathcal{N}(\boldsymbol{0}, \alpha_{\ell}^2\boldsymbol{I})$ and $\boldsymbol{b}_{\ell} \sim \mathcal{N}(\boldsymbol{0}, \beta_{\ell}^2\boldsymbol{I})$ for $\ell = 1, \ldots, L$.
    
    \medskip
    
    Each panel of Fig.~\ref{fig:mlp_priors} shows a sample of 2-layer, sigmoid activation function MLPs from a particular Gaussian prior.
    \begin{figure}
        \centering
        \includegraphics[scale=0.45]{mlp_priors}
...
\begin{frame}{Learning the Prior}
    In more general situations, there are a few approaches to choosing a prior that can be taken:
    \begin{itemize}
...
\begin{frame}{Monte Carlo Dropout}
    \textbf{Monte Carlo Dropout} is a simple, popular method for estimating the posterior predictive distribution. It works as follows:
    \begin{itemize}
        \item Before training, add dropout layers to the model
        \item When testing, repeat this $S$ times:
        \begin{itemize}
            \item Decide whether to drop a neuron by sampling from a $\text{Bernoulli}(p)$ distribution. Dropping out a neuron means setting the weights of its outgoing edges to zero.
            \item Compute the posterior predictive distribution.
        \end{itemize}
        \item Average the $S$ posterior predictive distributions.
    \end{itemize}
\end{frame}
...
\begin{frame}{The Effective Dimensionality of a Model}
    ...
    \begin{figure}
        \centering
        \includegraphics[scale=0.55]{effective_dimensionality_and_test_loss}
...
\begin{frame}{Double Descent}
    ...
    \begin{figure}
        \centering
        \includegraphics[scale=0.7]{double_descent}
...
\begin{frame}{Double Descent}
    ...
    \begin{figure}
        \centering
        \includegraphics[scale=0.45]{double_descent_and_bma}
...
\begin{frame}{Double Descent}
    The effective dimensionality helps explain why double descent occurs. In Fig.~\ref{fig:double_descent_and_effective_dimensionality}, as the layer width increases, the effective dimensionality decreases, increases, and then decreases again. The lower the effective dimensionality, the better the model generalizes. This suggests why the test error decreases, increases, and then decreases again.
    \begin{figure}
        \centering
        \includegraphics[scale=0.35]{double_descent_and_effective_dimensionality}
...
\begin{frame}{Binary Classification}
    ...
    \end{align*}
    Let $\boldsymbol{\Lambda} = -\nabla^2 \log p(\boldsymbol{y} | \boldsymbol{f}_X)$. Then $\nabla^2 \mathcal{L} = -\boldsymbol{\Lambda} - \boldsymbol{K}_{X, X}^{-1}$. Let $\hat{\boldsymbol{f}}$ be the MAP estimate. We can now compute a Laplace approximation to the posterior:
    \[
